With Consecutive_id AS (   
    ...
    WHERE S2.id = S1.id +1 AND S3.id = S1.id +2 AND S1.people >=100 AND S2.people >=100 AND S3.people >=100),
     ...
     S1 AS (
    SELECT S1_id AS id, S1_visitdate AS visit_date, S1_people AS people FROM Consecutive_id),
    
     S2 AS (
    SELECT S2_id AS id, S2_visitdate AS visit_date, S2_people AS people FROM Consecutive_id),
    
     S3 AS (            
    SELECT S3_id AS id, S3_visitdate AS visit_date, S3_people AS people FROM Consecutive_id)
